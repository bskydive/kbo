 ...
 * Сложность схемки(см. теорию графов, связность и пр)
 * длинна строк во всех конфигах
 * RPS
 * количество сервисов, умноженое на количество сред(prod/stage/qa) и количество команд
 * количество экосистем/подходов(bash/docker/compose/kuber), которые "исторически сложились"
 * падучесть инфры, кол-во и тип инцидентов(повторяются ли, отклаываются ли на потом)
 * размер базы знаний/доков
 * кто ещё лазит руками в конфиги и как часто?
 ...
 * [мифы о dev/urandom](https://habr.com/ru/company/mailru/blog/273147)
 	* они оба являются криптографически стойкими генераторами псевдослучайных чисел (CSPRNG). Различия между ними очень невелики и не имеют отношения к степени случайности.
