 ...
 * Разница между access_token и refresh_token
	...
	* refresh_token
		- для доступа в API при отсутствии сессии юзера
		— refresh_token хеш который хранится в базе данных
		- узнать refresh_token вы обычно не можете, так как он не гоняется через front channel.
		- Если ваши клиенты это серверные приложения, то у вас есть так называемый Client credentials flow. И вам не нужен рефреш в принципе. Просто смотрите за лайфтаймом токена и обновляете его по необходимости.
	* access_token
		— короткоживущий токен для доступа к ресурсу
		...
		- 		Если веб(SPA) или есть webview то implicit flow. Получаете access_token и вперед в API. Как протухнет — редирект за обновлением или тихонечко в айфрейме запросик.
	3. Работу с общей сессией лучше делегировать на ваш IdP/AS/OP по выбору.
	* Можно поковырять кишки вот тут. https://github.com/AzureAD/azure-activedirectory-library-for-js/tree/dev/lib
 ...
 * [Курс MIT «Безопасность компьютерных систем». Лекция 17: «Аутентификация пользователя», часть 1](https://habr.com/ru/company/ua-hosting/blog/429680/)
	...
	* радужная таблица - хэш-значение
	* соль - добавляется к значению, усложнение расшифровки хэша, при неизменном алгоритме или значении
	* challenge/response protocol - добавление challenge к хэшу пароля, защита от перебора hammering
	* защита от перебора - таймаут, белые/чёрные списки адресов
