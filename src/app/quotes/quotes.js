...
const quotes = [
	['Программисты', '#',
		...
		'На пасху все накрасились?',
		'какие у тебя предложения по снижению технического долга? У меня полный релиз этих предложений',
		...
		'Есть ещё груши в грушевницах!',
		'Показ проходит так: все чинно рассаживаются вокруг стола, тимлид достаёт ноутбук, и начинает всех удивлять',
		'Настало моё время офуительных историй в стиле {username}. Вот какой у нас теперь фольклор богатый. Будет чего джунам рассказывать.',
		'в выходные отдыхал так интенсивно, что даже поработать не удалось',
		'программист - проекту: соберись, тряпка!',
		'тут ядрёным Авгиевым буреломом запахло',
		'когда делаешь сео, то сразу отдельный широкополосный выделенный penetration uplink образуется. И ладно бы к менеджерам, они занятые. Так нет, к маркетологам, которым натурально нечего делать кроме как пенетрировать.'
	],
...
function showQuote() {
	...
	let quoteInd = getRandomNum(2, quotes[authorInd].length - 2); //0 - author, 1 - link
