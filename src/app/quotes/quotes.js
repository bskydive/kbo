...
// [[автор, URL, цитаты, цитаты, цитаты],[],[]]
const quotes = [
	[
		"Айтишники",
		"#",
		"Бизнесу не нужны процессы в голове, ему нужны деньги на кармане. Таким образом он готовит кадровый резерв. Если паровозик вдруг смог сам, то зачем ему машинист?",
		"Это просто - как два байта переслать.",
		...
		"тут пахнет Авгиевой конюшней",
		"когда делаешь SEO, то сразу отдельный широкополосный выделенный penetration uplink образуется. И ладно бы к менеджерам, они занятые. Так нет, к маркетологам, которым натурально нечего делать.",
		...
		'Надо постить скрины, желательно в распечатанном виде на лбу у конечного исполнителя',
		...
		'Эффект Да́ннинга — Крю́гера. Люди, имеющие низкий уровень квалификации, делают ошибочные выводы, принимают неудачные решения и при этом не способны осознавать свои ошибки',
		'красноглазить по пояс в консоли',
		'проект перманентно в огне и говне',
		...
		'Неполная документация не равна плохой. Это прототип, который работает, но делает меньше, чем должен.'
	],
	[
		"Царь Соломон",
		"http://www.adme.ru/svoboda-kultura/20-mudryh-myslej-carya-solomona-o-zhizni-886910/",
	...
	[
		"Конфуций",
		"http://www.adme.ru/svoboda-kultura/10-mudrejshih-citat-konfuciya-858410/",
	...
	[
		"Далай Лама",
		"http://www.adme.ru/svoboda-kultura/15-urokov-zhizni-ot-dalaj-lamy-858760/",
	...
	[
		"Фридрих Ницше",
		"http://www.adme.ru/tvorchestvo-pisateli/30-genialnyh-citat-mihaila-zhvaneckogo-867060/ © AdMe.ru",
	...
	[
		...
		"http://www.adme.ru/vdohnovenie-919705/ego-sto-let-odinochestva-zakonchilis-670955/ © AdMe.ru",
	...
	[
		"Будда",
		"http://www.adme.ru/svoboda-psihologiya/25-urokov-buddy-kotorye-pomogut-izbavitsya-ot-volnenij-1153760/",
	...
	[
		"Пётр Мамонов",
		"http://www.adme.ru/vdohnovenie/prostye-pravila-zhizni-ot-petra-mamonova-472805/",
	...
	[
		...
		"https://greatwords.org/authors/659/",
	...
	[
		...
		"https://ru.citaty.net/avtory/sun-tszy/",
...
function showQuote() {
	/**
	 * генератор случайных чисел, почти случайных
	 * @param result >= min
	 * @param result < max
	 */
	function getRandomNum(min, max) {
		...
		// резервный вариант
		let result = Math.floor(Math.random() * (max - min)) + min; //исключая max!!!
		...
		for (let i = 0; i < max; i++) {
			...
			cryptoResult = Math.abs(
				cryptoList[Math.floor(Math.random() * cryptoList.length)]
			);

			if (cryptoResult >= min && cryptoResult < max) {
				//случайное число попало в диапазон min..max
				result = cryptoResult;
				break;
			}
			if (i >= max) {
				console.log(
					"window.crypto.getRandomValues failed, switch to Math.random()"
				);
			}
		}
		return result;
	...
	let quoteInd = getRandomNum(2, quotes[authorInd].length); //0 - author, 1 - link
	if (
		document.querySelector("#id-p-quoteText").innerHTML ===
		quotes[authorInd][quoteInd]
	) {
		quoteInd = getRandomNum(2, quotes[authorInd].length); // repeat randomize for better usability without repeats
	}
	...
	document
		.querySelector("#id-a-quoteAuthor")
		.setAttribute("href", quotes[authorInd][1]);
	document.querySelector("#id-a-quoteAuthor").innerHTML =
		quotes[authorInd][0];

	quoteCopyText = `${quotes[authorInd][quoteInd]} ${quotes[authorInd][0]}.`;
}

function showCopyFailWarning() {
	window.alert("Не удалось скопировать в буфер обмена");
}

function fallbackCopyTextToClipboard(text) {
	try {
		var successful = document.execCommand("copy", false, text);
		if (!successful) {
			showCopyFailWarning();
		}
	} catch (err) {
...
function init() {
	...
	document.querySelector("#id_divHeaderText").innerHTML =
		" авторов: " +
	...
	document
		.querySelector("#id_getQuoteBtn")
		.addEventListener("click", showQuote);
	document
		.querySelector("#id_copyBtn")
